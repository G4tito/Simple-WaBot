...
const serialize = (msg, sock) => {
    ...
    msg.reply = async (text, opts = {}) => {
        ...
        if (opts.media) {
            const { mime, buffer } = await getFile(opts.media)
            ...
            delete opts.media;
            content = { [mtype]: buffer, caption: text, mimetype: mime, ...opts };
        } else if (opts.image || opts.video || opts.document || opts.sticker || opts.audio) {
