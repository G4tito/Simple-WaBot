...
function uploadToTelegraph(buffer) {
    return new Promise(async (resolve, reject) => {
        const { ext } = await fromBuffer(buffer);
        if (!/(jpg|jpeg|mp4|png)/i.test(ext)) return reject('The buffer must be of type image or video.');
