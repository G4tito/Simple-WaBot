...
const download = {
    V1: async function dl(url, opts) {
        ...
        const id = url.match(/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/)[1];

        const ress = await axios.get(url);
        const $ = cheerio.load(ress.data);
        const title = $('meta[property="og:title"]').attr('content');

        const res = await axios.post('https://convert.tubidi.buzz/ajax.php', new URLSearchParams({
            videoid: id,
            downtype: format,
            vquality: quality
        }).toString(), {
            headers: {
                ...
                'Referer': `https://convert.tubidi.buzz/?videoId=${id}`
            }
        });
